...
@section('script')
    ...
    <script type="text/javascript">
        ...
        function acciones(id_estado, id_curso, pac) {
            let buttons = seeButton(id_curso)

            @if(Auth::user()->isAdmin())
                buttons += editButton(id_curso) + deleteButton(id_curso)
            @endif
            ...
            datatable = $('#abm-table').DataTable({
                ...
                columns: [
                    ...
                    {title: 'Duracion', data: 'duracion'},
                    {
                        ...
                        render: function (data, type, row, meta) {
                            return data.map(function (tematica) {
                    ...
                    {
                        data: 'id_estado', name: 'id_estado', width: '20%',
                        render: function (data, type, row, meta) {
                            return acciones(data, row.id_curso, row.pac);
                        },
                        orderable: false
                    }
                ],

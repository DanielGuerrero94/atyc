<script type="text/javascript">
    $(document).ready(function () {
            ...
            callback: {
                ...
                onClick: function (node, a, item, event) {
                    profesor = '<tr>' +
                        ...
                        '<div class="btn btn-xs btn-danger quitar" data-id="' + item.id + '"><i class="fa fa-minus"></i></div>' +
                        '</td>' +
                        '</tr>';
                    existe = false;
                    ...
                    const form = $.merge($('#form-modificacion').serializeArray(), [{name: 'profesor_id', value: item.id}]);
                    $.ajax({
                        url: `{{url('cursos/')}}/${cursoId}/addProfesor`,
                        method: 'put',
                        data: form,
                        success: function (data) {
                            console.log(data)
                            console.log("success")
                            alert(`Profesor ${item.nombres} ${item.apellidos} agregado correctamente`)
                        },
                        error: function (error) {
                            console.log(error)
                            console.log("error")
                            alert("Ocurrio un error al agregar profesor")
                        }
                    })
        ...
        $('#profesores-del-curso').on('click', '.quitar', function (event) {
            ...
            const cursoId = {{$curso->id_curso}};
            const form = $.merge($('#form-modificacion').serializeArray(), [{name: 'profesor_id', value: profesorId}]);
            $.ajax({
                url: `{{url('cursos/')}}/${cursoId}/removeProfesor`,
                method: 'put',
                data: form,
                success: function (data) {
                    console.log(data)
                    console.log("success")
                    alert(`Profesor removido correctamente`)
                },
                error: function (error) {
                    console.log(error)
                    console.log("error")
                    alert("Ocurrio un error al remover profesor")
                }
            })
